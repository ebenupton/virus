...
x_smc_p1:     .byte MASK_LEFT, MASK_RIGHT     ; steep boundary mask
x_smc_col:    .byte $08, $F8                    ; steep column advance base
...
@x_setup:
    PHA                     ; save |dx|
    LDA x_smc_p1,X
    STA smc_t_boundary+1    ; steep boundary mask
    LDA x_smc_col,X
    DEC A                   ; $08→$07 or $F8→$F7
    STA smc_t_col_adv+1     ; steep column advance
    BRA @do_dy
...
@shallow:
    ...
    ; Route by Y direction then X direction
    LDA y0
    CMP y1
    BCS @s_yup
    ; Y down:
    LDA x1
    CMP x0
    BCS sdr_align            ; x1 >= x0 → right
    BRA sdl_align            ; x1 < x0 → left
@s_yup:
    LDA x1
    CMP x0
    BCC @s_yleft
    JMP sur_align            ; x1 >= x0 → right
@s_yleft:
    JMP sul_align            ; x1 < x0 → left

@zero_line:
    PLY                     ; restore sub-row (balance stack)
    RTS

; === SHALLOW Y-down X-right pair loop ===

sdr_align:
    LDA mask_zp
    CMP #MASK_LEFT           ; P1 mask for right-going
    SEC
    BEQ sdr_pair_loop
    BRA sdr_p2
...
; === SHALLOW Y-down X-left pair loop ===

sdl_align:
    LDA mask_zp
    CMP #MASK_RIGHT          ; P1 mask for left-going
    SEC
    BEQ sdl_pair_loop
    BRA sdl_p2
...
; === SHALLOW Y-up X-right pair loop ===
