...
x_smc_p1:     .byte MASK_LEFT, MASK_RIGHT     ; P1 mask / steep boundary
x_smc_p2:     .byte MASK_RIGHT, MASK_LEFT      ; P2 mask
x_smc_col:    .byte $08, $F8                    ; column advance
...
draw_line:
    ...
    EOR #$FF
    INC A
    LDX #1                  ; table index: left-going
    BRA @x_setup

@x_right:
    ; X going right: |dx| = x1 - x0
    LDX #0                  ; table index: right-going

@x_setup:
    PHA                     ; save |dx|
    ; P1 mask = steep boundary mask
    LDA x_smc_p1,X
    STA smc_su_p1+1
    STA smc_sd_p1+1
    ...
    STA smc_su_p2+1
    STA smc_sd_p2+1
    ; Column advance (shallow + derive steep)
    LDA x_smc_col,X
    STA smc_su_col+1
    STA smc_sd_col+1
    DEC A                   ; $08→$07 or $F8→$F7
    STA smc_t_col_adv+1
    ; Carry branch opcode
    LDA x_smc_bra,X
    STA smc_su_bcc
    STA smc_sd_bcc
    ; Carry INC/DEC opcode
    LDA x_smc_carry,X
    STA smc_su_carry
    STA smc_sd_carry
    BRA @do_dy
...
@shallow:
    ...
    LDA y0
    CMP y1
    BCS su_align           ; y0 >= y1 → Y up (or flat)
    ; Y down: check entry alignment
    LDA mask_zp
    CMP smc_sd_p1+1         ; is mask_zp == P1 mask?
    SEC
    BEQ sd_pair_loop         ; aligned → enter at P1
    BRA sd_p2                ; misaligned → enter at P2
...
smc_sd_p1:
    ...
    INY
    CPY #8
    SEC                     ; restore C for P2's SBC (after CPY)
    BNE sd_no_y1
    INC base+1
    INC base+1
    LDY #0
sd_no_y1:
    ...
    BEQ sd_done
sd_p2:
    ; P2: second pixel of pair
    LDA (base),Y
smc_sd_p2:
    EOR #MASK_RIGHT         ; SMC: MASK_RIGHT (right) or MASK_LEFT (left)
    STA (base),Y
    ; Bresenham Y step
    TXA
    SBC delta_minor
...
smc_sd_col:
    ...
    STA base
smc_sd_bcc:
    BCC sd_no_page          ; SMC: BCC ($90) right, BCS ($B0) left
smc_sd_carry:
    INC base+1              ; SMC: INC ($E6) right, DEC ($C6) left
sd_no_page:
    SEC
    DEC pixel_count
    BNE sd_pair_loop
...
su_align:
    ; Y up: check entry alignment
    LDA mask_zp
    CMP smc_su_p1+1         ; is mask_zp == P1 mask?
    SEC
    BEQ su_pair_loop         ; aligned → enter at P1
    BRA su_p2                ; misaligned → enter at P2
...
smc_su_col:
    ...
    STA base
smc_su_bcc:
    BCC su_no_page          ; SMC: BCC ($90) right, BCS ($B0) left
smc_su_carry:
    INC base+1              ; SMC: INC ($E6) right, DEC ($C6) left
su_no_page:
    SEC
    ...
    BNE su_pair_loop
su_done:
    ; Fall through to s_exit

s_exit:
    ; Set mask_zp for chaining based on x1 parity
    LDA x1
    LSR A
    LDA #MASK_LEFT
    BCC @s_set
