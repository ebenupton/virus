...
Byte format: [color:1][color:1][color:1][height:5]
  - Bits 0-4: height (0..31), 0 = sea level
  - Bits 5-7: edge colour pattern, indexed into edge_color_lut
...
def main():
    ...
    # Step 7b: Enforce max slope of 4 between adjacent cells (for interp LUT)
    MAX_SLOPE = 4
    changed = True
    passes = 0
    while changed:
        ...
        for y in range(SIZE):
            for x in range(SIZE):
                for dy, dx in [(0,1),(0,-1),(1,0),(-1,0)]:
                    ...
                    if heights[y][x] - heights[ny][nx] > MAX_SLOPE:
                        heights[y][x] = heights[ny][nx] + MAX_SLOPE
                        changed = True
        passes += 1
    ...
    while changed:
        ...
        for y in range(SIZE):
            for x in range(SIZE):
                byte = prev[y][x]
                # Bit 6: check x-direction neighbours
                b6 = (byte >> 6) & 1
                b6_l = (prev[y][(x - 1) % SIZE] >> 6) & 1
                b6_r = (prev[y][(x + 1) % SIZE] >> 6) & 1
                if b6 != b6_l and b6_l == b6_r:
                    byte = (byte & ~(1 << 6)) | (b6_l << 6)
                    changed = True
                # Bit 7: check z-direction neighbours
                b7 = (byte >> 7) & 1
                b7_u = (prev[(y - 1) % SIZE][x] >> 7) & 1
                b7_d = (prev[(y + 1) % SIZE][x] >> 7) & 1
                if b7 != b7_u and b7_u == b7_d:
                    byte = (byte & ~(1 << 7)) | (b7_u << 7)
                    changed = True
                packed[y][x] = byte
    ...
    for y in range(SIZE):
        for x in range(SIZE):
            ...
            pattern = (byte >> 5) & 7
            if pattern == 0b010:  # 010 → 000: clear bit 6
                packed[y][x] = byte & ~0x40
            elif pattern == 0b100:  # 100 → 110: set bit 6
                packed[y][x] = byte | 0x40
    ...
    with open("map_data.inc", "w") as f:
        ...
        f.write("; Byte format: [color:1][color:1][color:1][height:5]\n")
        f.write("; Bits 5-7 = edge colour pattern for edge_color_lut\n\n")
        f.write("height_map:\n")
        for y in range(SIZE):
    ...
    with open("map_data.inc", "a") as f:
        f.write("\n; 16x32 pixel minimap in MODE 2 character-cell format (256 bytes)\n")
    ...
    water = sum(1 for y in range(SIZE) for x in range(SIZE)
                if (packed[y][x] & 0x1F) == 0)
    max_h = max(packed[y][x] & 0x1F for y in range(SIZE) for x in range(SIZE))
